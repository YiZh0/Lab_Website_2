...
function loadStatsPage() {
	...
	for (i = 2; i < table1.rows.length; i++) {
		if (parseInt(table1.rows[i].cells[2].innerHTML) > parseInt(table1.rows[i].cells[3].innerHTML)) {
			wins++;
			table1.rows[i].cells[4].innerHTML = "Cu Boulder";
		}
		else {
			table1.rows[i].cells[4].innerHTML = table1.rows[i].cells[1].innerHTML;
		}
	}
	document.getElementById("wins").innerHTML = wins;
	document.getElementById("losses").innerHTML = table1.rows.length - wins - 2;
}
...
function loadPlayersPage() {
	...
	for (j = 0; j < players.length; j++) {
		yy = players[j].name;
		ti.push('<a class="dropdown-item" href="#" onclick="switchPlayers('+j+')">'+yy+'</a>');

	}
	document.getElementById("player_selector").innerHTML = ti.join(" ");
}

function switchPlayers(playerNum) {
	if (playerNum > players.length) {
		return function () {
			alert('No This Player');
		}
	}
	...
	document.getElementById("avg_p_yards").innerHTML = Math.round(players[playerNum].pass_yards/gp);
	document.getElementById("avg_r_yards").innerHTML = Math.round(players[playerNum].rushing_yards/gp);
	document.getElementById("avg_rec_yards").innerHTML = Math.round(players[playerNum].receiving_yards/gp);
}
